### the trouble is that our Y is on level 1 and X and M are on level 2
### that means we'd like to move Y into level 2, which means modeling the Y intercepts
...
for (tbl in tbl_names) {
  for (mem in memType) {
    for (ROI_name in ROI_name_variable) {
      ...
      Y~1
      level:2
      Y ~ b1 * M1 + b2 * M2 + b3 * M3 + b4 * M4 + b5 * M5 + b6 * M6 + b7 * M7 + b8 * M8 + c * X + d1 * M1 * X + d2 * M2 * X + d3 * M3 * X + d4 * M4 * X + d5 * M5 * X + d6 * M6 * X + d7 * M7 * X + d8 * M8 * X
      ...
      totalIndirect := ab1 + ab2 + ab3 + ab4 + ab5 + ab6 + ab7 + ab8 + (a1 + a2 + a3 + a4 + a5 + a6 + a7 + a8) * c
...
# # error term for the latent variable approach is higher. Our model fits are likely worse, nor better
# model221_latentVar <- '
# level:1
# Y~1
# level:2
# # Latent variable definition
# latentMediator =~ M1 + M2 + M3 + M4 + M5 + M6 + M7 + M8
# 
# # Paths
# Y ~ b*latentMediator + c*X
# 
# latentMediator~a*X
# 
# #indirect and total effects
...
# 
# # Total effect
# totalEffect := indirectEffect + c
# '
# 
# fit221<-sem(model221_latentVar,data=Data_mixed_clean ,cluster="ItemID")
# summary(fit221,fit.measures=TRUE)
