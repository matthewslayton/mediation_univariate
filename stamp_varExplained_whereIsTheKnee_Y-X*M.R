  ...
  for (mem in memType) {
    ...
    for (ROI_name in ROI_name_variable) {
     ...
     for (i in 1:length(top_factors)) {
        ...
        # Generate interaction terms between X and each selected factor
        interaction_terms <- paste0("X*", selected_factors)
        
        # Combine the interaction terms into a single string, separated by " + "
        interactions_combined <- paste(interaction_terms, collapse = " + ")
        
        # Construct the model formula for cumulative interactions
        cumulative_formula <- as.formula(paste0("Y ~ ", interactions_combined))
        
        # Fit the linear model with the cumulative set of interaction terms
        cumulative_model <- lm(cumulative_formula, data = updated_data_mixed_clean)
...
file_path <- "/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/varExplained_R2change_Y-X*M.xlsx"
