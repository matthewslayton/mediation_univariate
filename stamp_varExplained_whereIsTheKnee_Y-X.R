  ...
  for (mem in memType) {
    ...
    for (ROI_name in ROI_name_variable) {
     ...
     # how_many_fac <- 228 #50 #100 #228 #67, 8
      ...
      df <- model_summary$df[1] + model_summary$df[2]

      # Extract p-value for the predictor (X)
...
#pval <- do.call(rbind, lapply(pval, data.frame, stringsAsFactors = FALSE))
...
file_path <- "/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/varExplained_R2_Y-X.xlsx"
